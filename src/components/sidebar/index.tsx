...
export const Sidebar = () => {
  ...
  return (
    <>
      <aside
        className={`hidden md:flex flex-col h-full relative bg-primary transition-all duration-300 transition- text-gray-200 px-4 py-6  ${className}`}
      >
        ...
        <nav className="flex flex-col gap-y-2 mt-[100px] overflow-hidden">
          {links.map((link) => (
            ...
            >
              ...
              <span className={isOpen ? "" : "hidden"}>{link.text}</span>
            </Link>
          ))}
        </nav>
        ...
        <div className="mt-auto mb-0">
          ...
          <div className="text-sm mt-4 text-center">Versão 1.0.0</div>
        </div>
      </aside>
    </>
  );
