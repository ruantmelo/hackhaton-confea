...
export function SignInForm(){
  ...
  const onSubmit = async ({login, senha}: FormValues) => {
    console.log('REALIZANDO O LOGIN ', login)
  ...
  return (
    <div >
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col max-w-sm mx-auto gap-6 ">
        <p className='text-destructive'>{form.formState.errors.api_error?.message}</p>
        <FormField
        ...
        <Button disabled={form.formState.isSubmitting} type="submit">
          {form.formState.isSubmitting ? <LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> : "Entrar"}
        </Button>
      </form>
    </Form>
    </div>
